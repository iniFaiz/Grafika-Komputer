...
texture_id_floor = 0 # New global for floor texture ID

# Initial ball state (can be grouped for clarity or kept as is)
initial_ball_pos_x = 0.0
initial_ball_pos_y = 2.0
initial_ball_pos_z = 0.0

# HUD Input Mode Variables
is_hud_input_mode = False
hud_input_strings = ["0.0", "0.0", "0.0"] # Stores current text for Vx, Vy, Vz
hud_input_current_field = 0 # 0 for Vx, 1 for Vy, 2 for Vz
...
def create_checkerboard_texture():
    ...
    # Mipmapping is important for anisotropic filtering to work well
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR)

    # Check for and apply anisotropic filtering
    extensions = glGetString(GL_EXTENSIONS)
    if extensions and b'GL_EXT_texture_filter_anisotropic' in extensions:
        try:
            # These constants might be directly available or via OpenGL.GL.EXT.texture_filter_anisotropic
            # For PyOpenGL, they are often directly in OpenGL.GL if the driver exposes the extension.
            GL_TEXTURE_MAX_ANISOTROPY_EXT = 0x84FE
            GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT = 0x84FF
            
            max_anisotropy = glGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT)
            # glGetFloatv in PyOpenGL often returns a list/array
            if isinstance(max_anisotropy, (list, tuple)):
                max_anisotropy = max_anisotropy[0]
            
            glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, max_anisotropy)
            print(f"Anisotropic filtering enabled. Max anisotropy: {max_anisotropy}")
        except GLError as e:
            print(f"Error enabling anisotropic filtering: {e}. Constants might not be defined if extension is present but not fully loaded by PyOpenGL's core.")
        except NameError:
...
def draw_floor():
    ...
    # Set material color to white when using GL_MODULATE with textures
    # so the texture colors are not overly darkened by a dark material.
    # Lighting will still affect the final color.
    glColor3f(0.8, 0.8, 0.8) # A slightly off-white can look good

    # Number of times to repeat the texture across the floor
    ...
    # Lighting was not disabled here before, so no need to re-enable explicitly for the floor
    # If you had glDisable(GL_LIGHTING) for the floor, you'd re-enable it.
...
def init():
    ...
    create_checkerboard_texture() # Call the new texture creation function
    # Set texture environment to modulate, so lighting affects texture
    ...
    if is_hud_input_mode:
        ...
        for i in range(3):
            ...
            if i == hud_input_current_field:
                value_str += "_" # Simple cursor indicator
...
def display():
    ...
    glLoadIdentity()
    update_camera() # Matrix ModelView sekarang adalah matrix View (V)

    # Atur posisi cahaya SETELAH update_camera() untuk cahaya dunia
    # Posisi/arah yang diberikan di sini adalah dalam koordinat DUNIA.
    # OpenGL akan mengalikannya dengan matrix ModelView saat ini (yaitu V)
    # untuk mendapatkan posisi/arah cahaya dalam ruang mata.
    world_light_direction = [1.0, 1.0, 1.0, 0.0] # Contoh: cahaya dari arah (1,1,1) dunia
    glLightfv(GL_LIGHT0, GL_POSITION, world_light_direction)
    # Anda mungkin perlu menghapus panggilan glLightfv dari fungsi init() jika melakukan ini.
...
def keyboard(key, x_mouse, y_mouse):
    ...
    if is_hud_input_mode:
        ...
        if key == b'\r': # Enter key
            # Validate current field (e.g. if empty, set to "0.0")
            ...
            if hud_input_current_field > 2: # Confirmed all fields
                try:
            ...
            else: # Moved to next field, ensure it has a default if empty
    ...
    # Normal keyboard controls if not in HUD input mode
...
def timer(value):
    ...
    # is_aiming_throw (which is true during HUD input) already handles pausing physics
    ...
    elif is_aiming_throw: # This block is active during HUD input and mouse aiming
        vel_x, vel_y, vel_z = 0.0, 0.0, 0.0
        spin_speed = 0.0
        # Keep ball at the position it was when aiming/HUD input started
        pos_x = throw_start_ball_pos_x
        pos_y = throw_start_ball_pos_y
        pos_z = throw_start_ball_pos_z
...
init()
# Initialize throw_start_ball_pos with current ball position
# This is important if 't' is pressed before any mouse interaction
throw_start_ball_pos_x = pos_x
throw_start_ball_pos_y = pos_y
throw_start_ball_pos_z = pos_z
